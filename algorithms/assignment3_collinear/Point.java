...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        return calculateSlope(this, that);
    }

    private static double calculateSlope(Point self, Point that) {
        if (that.x == self.x && that.y == self.y) {
        ...
        return (that.y - self.y) / (1.0 * (that.x - self.x));
    }
    ...
    public int compareTo(Point that) {
        ...
        // y points are equal, lets check for x
        else if (this.x < that.x) {
            return -1;
        } else if (this.x > that.x) {
            return 1;
        } else {
            // Both points have same coordinates
            return 0;
        }
    }
    ...
    public Comparator<Point> slopeOrder() {
        ...
        return new Comparator<Point>() {
            public int compare(Point p1, Point p2) {
                double slopeToP1 = calculateSlope(self, p1);
                double slopeToP2 = calculateSlope(self, p2);
